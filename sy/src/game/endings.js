...
export const ENDINGS = [
  // ── 즉시 끝나는 것들 ───────────────────────────────────────────
  {
    ...
    hint: '불 꺼진 공장에서 밸브를 돌리면',
    test: (s) => s.has('valve') && s.has('blackout'),
    lines: ['압력계 바늘이 오른쪽 끝까지 갔다.',
      '정전된 공장에서 경보는 울리지 않았다.',
      '양촌 하늘이 잠깐 아주 밝았다.'],
  },
  {
    id: 'E02', title: '자수', tag: 'odd', when: 'any',
    hint: '파출소에서 하지 않아도 될 말을 하면',
    test: (s) => s.has('confess'),
    lines: ['"제가 그랬습니다."',
      '경관은 무엇을 그랬는지 세 번 물었다.',
      '조서에는 결국 출근 시각만 적혔다.'],
  },
  {
    id: 'E03', title: '과다복용', tag: 'bad', when: 'any',
    hint: '같은 약을 세 번 받으면',
    test: (s) => s.count('pill') >= 3,
    lines: ['세 번째 알약을 삼키자 복도가 기울었다.',
      '병원 침대는 생각보다 편했다.',
      '출근은 내일부터 하기로 했다.'],
  },
  {
  ...
  {
    id: 'E23', title: '종소리 배웅', tag: 'good', when: 'goal',
    hint: '교회 종을 치고 나오면',
    test: (s) => s.has('church_bell'),
    lines: ['한 번만 치려던 종을 세 번 쳤다.',
      '소리가 구래동 위로 넓게 퍼졌다.',
      '누가 배웅해 준 것 같은 아침이었다.'],
  },
  {
    id: 'E24', title: '굴뚝 연기', tag: 'true', when: 'goal',
    hint: '망원경으로 본 것을 옥상에서 다시 보면',
    test: (s) => s.has('saw_smoke') && s.has('rooftop'),
    lines: ['호수공원 망원경으로 본 연기가 옥상에서도 보였다.',
      '방향이 조금 달랐다. 두 번 보면 알 수 있다.',
      '공단에 도착해 물었더니, 오늘은 아무 공정도 돌리지 않았다고 했다.'],
  },
  {
    id: 'E25', title: '냉동식품', tag: 'odd', when: 'goal',
    hint: '마트 냉동고를 열어 보면',
    test: (s) => s.has('frozen'),
    lines: ['냉동고 문을 오래 열어 두었다.',
      '가방 속 만두는 공단에 도착할 때쯤 다 녹았다.',
      '점심으로는 못 쓰게 됐다.'],
  },
  {
    id: 'E26', title: '등기우편', tag: 'good', when: 'goal',
    hint: '편지를 넣고 우체국에도 들르면',
    test: (s) => s.has('letter') && s.has('parcel'),
    lines: ['우체통에 넣은 편지와 창구에서 부친 소포.',
      '둘 다 오늘 안에 떠난다고 했다.',
  ...
  },
  {
    ...
    hint: '무사히 도착하면',
    test: () => true,
    lines: ['구래동에서 양촌공단까지 걸었다.',
      '논과 아파트와 공장이 차례로 지나갔다.',
      '이제 여기서 일한다.'],
  },
];
