...
import { pathBounds, pointInPath } from '../src/world/geo.js';
...
const goal = map.regions.find((r) => GAME.goalRegions.includes(r.id));
const gb = pathBounds(goal.path);

// 탐색 범위 — 출발점과 목적지를 넉넉히 감싸는 사각형
const pad = 200;
const minX = Math.floor(Math.min(start.x, gb.minX) - pad);
const maxX = Math.ceil(Math.max(start.x, gb.maxX) + pad);
const minY = Math.floor(Math.min(start.y, gb.minY) - pad);
const maxY = Math.ceil(Math.max(start.y, gb.maxY) + pad);
...
console.log(`출발 (${start.x}, ${start.y}) → 목적지 ${goal.name}`);
